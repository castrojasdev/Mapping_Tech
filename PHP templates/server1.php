...
if (isset($_POST['reg_user'])) {
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO userr (username, email, password) 
...
if (isset($_POST['suggest_idea'])) {
  ...
  if (empty($bug)) { array_push($errors, "Any bugs!!!!!"); }
  	...
  	$queryy = "INSERT INTO suggest (idea, des, bug) 
...
if (isset($_POST['send'])) {
  ...
  if (empty($msg)) { array_push($errors, "Give a message"); }
  	...
  	$queryy =" INSERT INTO `contacts`(`Email`, `Name`, `Subject`, `Message`)
...
if (isset($_POST['cab-book'])) {
  ...
  if (empty($bug)) { array_push($errors, "Time!!!!!"); }
  	...
  	$queryy = "INSERT INTO `cab-books`(`Pickup`, `Drop`, `Time`) 
  			  VALUES('$idea', '$desp', '$bug')";
  	mysqli_query($db, $queryy);
 ...
 if (isset($_POST['book'])) {
  ...
  if (empty($date2)) { array_push($errors, "Date!!!!!");}
    ...
    $queryy =" INSERT INTO `datee`(`Name`, `Number`, `Mail`, `Start-data`, `End-date`)
 ...
 if (isset($_POST['rent-book'])) {
  ...
  if (empty($date2)) { array_push($errors, "Date!!!!!");}
    ...
    $queryy =" INSERT INTO `rentt`(`City`, `Time`, `Start-date`, `End-date`) 
 ...
 if (isset($_POST['hotel'])) {
  ...
  if (empty($date2)) { array_push($errors, "Date!!!!!");}
    ...
    $queryy =" INSERT INTO `hotel`(`Name`, `Check-in`, `Check-out`, `Room`) 
...
if (isset($_POST['b-book'])) {
  $bank = mysqli_real_escape_string($db, $_POST['pic']);
  $card = mysqli_real_escape_string($db, $_POST['drop']);
  $cvv = mysqli_real_escape_string($db, $_POST['time']);

  if (empty($bank)) {
      array_push( $errors, "Pickup is required");
  }
  if (empty($card)) {
      array_push($errors, "dropis required");}
  ...
  if (empty($cvv)) {
    array_push($errors, "Time is required");}
  	...
  	$queryy = "INSERT INTO `balaji`(`Pickup`, `Drop`, `Time`) 
  			  VALUES('$idea', '$desp', '$bug')";
  	mysqli_query($db, $queryy);
     ...
     if (isset($_POST['book-bike'])) {
      ...
      if (empty($date2)) { array_push($errors, "Date!!!!!");}
        ...
        header('location: booking.php');
          ...
          ///////////////bikerental1//////echo '<script>';
          $db2 = mysqli_connect('localhost', 'root', '', 'bike');

          if (isset($_POST['castro'])) {
           // receive all input values from the form
           $ide = mysqli_real_escape_string($db2, $_POST['name']);
           $des = mysqli_real_escape_string($db2, $_POST['phone']);
           $bu = mysqli_real_escape_string($db2, $_POST['bikemail']);
           $date1 = date('Y-m-d',strtotime( $_POST['date1']));
           $date2 = date('Y-m-d',strtotime($_POST['date2']));
           ...
           if (empty($date2)) { array_push($errors, "Date!!!!!");}
             ...
             $queryy =" INSERT INTO `book`(`name`, `number`, `mail`, `Start-date`, `End-date`)
             VALUES('$ide', '$des', '$bu','$date1','$date2')";
             mysqli_query($db2, $queryy);
             echo '<script>';
             echo 'var ask = 1;';
